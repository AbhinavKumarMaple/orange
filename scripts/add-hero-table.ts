/**
 * One-time migration: create hero_content table and seed initial row.
 * Run with: npx tsx scripts/add-hero-table.ts
 */
...
async function run() {
  ...
  const existing = await db.execute(sql`SELECT id FROM hero_content LIMIT 1`);
  if (existing.rows.length === 0) {
    console.log("Seeding initial hero row...");
    await db.execute(sql`
